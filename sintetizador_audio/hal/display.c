...
// FUNÇÃO PARA EXIBIR O LOGO UMA VEZ E RECONFIGURAR O DISPLAY
void display_show_logo(void) {
  
    ssd1306_t logo_display;

    ssd1306_init_bm(&logo_display, LOGO_WIDTH, LOGO_HEIGHT, false, ssd1306_i2c_address, i2c1);

    ssd1306_config(&logo_display);

    ssd1306_draw_bitmap(&logo_display, logo_bitmap);

    free(logo_display.ram_buffer);
